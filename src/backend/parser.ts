import { Statement, Program, Expression, BinaryExpr, NumericLiteral, Identifier, VariableDeclaration, AssignmentExpr, Property, ObjectLiteral, AbsExpr, CallExpr, MemberExpr, FunctionDeclaration} from './ast'
...
export default class Parser {
    ...
    private parseFunctionDeclaration(): Statement {
        this.advance()
        const name = this.expect(TokenType.Identifier, "Expected name after 'fn' keyword.").value
        ...
        for (const arg of args) {
            if (arg.kind !== "Identifier") {
                console.log(arg)
                throw "Expected Paramaters inside the function to be of type 'string'."
            }

            paramaters.push((arg as Identifier).symbol)
        }
        ...
        while (this.at().type !== TokenType.EOF && this.at().type !== TokenType.RightBrace) {
    ...
    private parseArgumentsList(): Expression[] {
        ...
        while (this.notEOF() && this.at().type == TokenType.Comma && this.advance()) {
